...
- **Working site search** — WordPress answers `/?s=term` dynamically, and every static host silently returns the *homepage* for it instead. The exporter builds a search index from the crawled pages (`/search-index.json`), generates a themed results page (`/suche/` on German sites, `/search/` otherwise — cloned from the site's own 404 page so it inherits the theme CSS, `noindex,follow` like WordPress), points the theme's own search forms and the JSON-LD `SearchAction` at it, and redirects any surviving `?s=` URL there before first paint. Diacritics- and case-insensitive (`strasse` finds `Straße`), snippets with the term highlighted, no JS library. Disable with `--no-search`, override the path with `--search-path`, cap the index with `--search-max-chars`. The export must be served over HTTP (the index cannot be fetched from `file://`).
...
| `search` | client-side site search: index, themed results page, form/JSON-LD rewiring (`--no-search`, `--search-path`, `--search-max-chars`) |
