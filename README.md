...
- **Generated sitemap** — a fresh `/sitemap.xml` is written from the URL set the origin sitemap declared and the export contains: `noindex` pages, redirect sources and canonical mismatches are excluded, pages only discovered via links stay out unless you pass `--sitemap-include-linked`, and **machine-generated artifact pages are dropped**: WordPress gives every uploaded file its own attachment page, image plugins add cache directories (phpThumb & co.), and a plugin that registers a taxonomy for its own bookkeeping leaves an **empty term archive** per term — a page whose entire content is WordPress' "Nothing found" template, which Yoast nevertheless declares `index, follow`. Yoast's *attachment sitemap* happily lists the first kind too, so "the origin declared it" is no proof of content. All of them are still exported (a lightbox or a stray link may point at them) but appear in neither `sitemap.xml` nor the site search; `--list-attachment-pages` / `--list-empty-archives` list them anyway. `<lastmod>` comes from the `Last-Modified` header. The `robots.txt` `Sitemap:` line is pointed at it (a missing robots.txt is generated). `--no-generate-sitemap` keeps the origin sitemap files instead (unchanged except the localized XSL reference). After a domain move, submit `/sitemap.xml` in Search Console once.
...
**The index** is built during the crawl: every page contributes `{path, title, meta description, main-content text}` from the soup the core already parsed, so it costs no extra request. It runs through the same "is this a real page" classifier as the generated sitemap — no error pages, no redirect stubs, no attachment/cache artifacts, nothing that declares another URL canonical — with one deliberate difference: `noindex` and "the origin sitemap did not list it" are instructions to *search engines*, and WordPress' own site search returns such pages, so the exported search does too. The result is written to `/search-index.json` and **always fetched as that separate file** — one download the browser caches across searches and across pages, rather than a copy of the index baked into every results page. The cards can therefore arrive after the theme's grid code has initialized, so the renderer tells the grid to measure them — `reloadItems` followed by the command that engine actually needs (`arrange` for Isotope, whose `layout` only re-places the items an earlier filter pass selected; `layout` for Masonry and Packery), plus the `debouncedresize` event themes use when they switch the grid's own resize handling off. It does so repeatedly: immediately, in a fresh task, on DOM-ready, on `load` and once the cards' images have loaded, because a grid measured before styles and thumbnails settle puts every card on the same spot.

**A site with no search gets none.** If not one exported page offers a way *into* a search — no search box, no internal `?s=` link — the results page, the index, the probes and the `?s=` redirect are all skipped: nothing would lead there. **The theme's 404 template counts**, and is often the last place a search box survives: left unwired it would submit to `/?s=term`, which every static host answers with the homepage. `--force-search` builds the page even without any entry point (for a theme that creates its search box in JavaScript, or when you link the path yourself).
...
| `--list-attachment-pages` | list WP attachment/image-cache pages in sitemap.xml and the search (default: excluded as artifacts) |
| `--list-empty-archives` | list category/tag/taxonomy archives whose loop is empty (default: excluded as artifacts) |
...
| `search` | client-side site search: index, results page harvested from the live `/?s=` design, form/JSON-LD rewiring (`--no-search`, `--no-search-harvest`, `--search-path`, `--search-max-chars`) |
...
| `wordpress` | WP head-cruft removal, artifact-page detection (attachment/image-cache pages, empty term archives), WPForms AJAX skip, WooCommerce cart/checkout skip |
...
| `wordfence` | Live-Traffic beacon removal (stops the `/?wordfence_lh=…` visitor requests) |
